...
PROFILES_OUT=$CapstoneFetch/soc-pokec-profiles-XIANGMIN_PG-tabular.txt
RELATIONS_OUT=$CapstoneFetch/soc-pokec-relationships-XIANGMIN_PG-tabular.txt
...
sudo -u XIANGMIN_PG $XIANGMIN_PG/bin/psql -d "$CAP_DBNAME" -c "COPY profiles FROM '$PROFILES_OUT' WITH DELIMITER E'\t' CSV HEADER NULL AS 'null';"
